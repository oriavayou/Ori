...
**הלוגו**: שים את קובץ הלוגו ב-`promo/brand/logo.png`. אם הרקע שלו לבן:

```bash
node tools/prep-logo.mjs      # מסיר את הרקע הלבן וחותך את השוליים → brand/logo-alpha.png
```

הסרטון משתמש בו בשתי נקודות — ברגע המעבר (הקו שרץ לאורך הסרטון "מוסר" את
המקום ללוגו) ובסיום. בלי קובץ לוגו הכל נופל חזרה לטיפוגרפיה.
