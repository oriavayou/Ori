...
סרטון עם **סיפור**, במבנה קלאסי: הוק → מסר → אמינות → מה נכנס לתמונה → תועלת
מספרית → קריאה לפעולה. הטקסט נושא את הטיעון, והאתר מופיע בין המשפטים כהוכחה —
לא כנושא. ~37 שניות.

**צילומים ידניים**: אם צילומי המסך מגיעים מהטלפון ולא מהכלי, שמים אותם ב-`promo/shots/`
וכותבים `shots.json` ידנית — `views.phone.w/h` הם רק היחס, אז אפשר לתת את מידות
הפיקסלים של הקובץ. כדאי לחתוך קודם את שורת הכתובת של הדפדפן:

```bash
ffmpeg -i screen.png -vf "crop=iw:ih-186:0:186" -q:v 3 promo/shots/real-1.jpg
```

**הלוגו** — שני קבצים אפשריים ב-`promo/brand/`:

| קובץ | לאן זה הולך |
| --- | --- |
| `logo-sign.png` | גרסת השלט המואר. ממלאת את המסך ברגע המעבר, ובמסגרת בסיום. זו הגרסה המומלצת — היא אווירה, לא אייקון. |
| `logo.png` | הגרסה השטוחה. אם הרקע לבן, `node tools/prep-logo.mjs` מסיר אותו וחותך שוליים → `logo-alpha.png` |

הסרטון מעדיף את `logo-sign.png`; בלי אף קובץ הכל נופל בחזרה לטיפוגרפיה, בלי לשבור
כלום. בכל מקרה — **קבצי המותג לא נשמרים בגיט**, הם של הלקוח.
